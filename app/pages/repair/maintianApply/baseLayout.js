...
class LendLayout extends React.Component {
  ...
  render() {
    ...
    const update = (row) => {
      ...
      getMaintianById({ id: row.id }).then((res) => {
        this.setState({
          taskInfo: res.data,
          records: row,
        });
        let lsit = res.data.limsRepairmaintainapplyitemList?.map((item) => ({
          ...item,
          ...item.limsRepairmaintainapply,
        }));

        row = {
          ...res.data.limsRepairmaintainapply,
          limsRepairmaintainapplyitemSaveDTOS: lsit,
        };
        formatList.map((item) => {
    ...
    const submitFlow = () => {
      ...
      let deviceList = formData.limsRepairmaintainapplyitemSaveDTOS.map(
        (item) => {
      ...
      let submitValue = {
        ...formData,
        submitType: 1,
        limsRepairmaintainapplyitemSaveDTOS: deviceList,
        totalFee: totalFee,
    ...
    const onFinish = (values) => {
      ...
      let deviceList = values.limsRepairmaintainapplyitemSaveDTOS.map(
        (item) => {
